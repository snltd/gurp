...
pub fn configure(host_config: &HostConfig, opts: &Opts) -> anyhow::Result<bool> {
    ...
    for module in &host_config.modules {
        verbose!(opts, "Running module '{}'", module);
    }
...
mod test {
    ...
    fn test_define_host_config() {
        ...
        assert_eq!(
            ...
            define_host_config(&mut janet_runner::janet_client(), user_input).unwrap()
        );
    }
    ...
    fn test_define_host_config_no_modules() {
        ...
        assert!(define_host_config(&mut janet_runner::janet_client(), user_input).is_err());
    }
